...
public class J1979Client
{
    private ICanBus _canBus;

    public J1979Client(ICanBus canBus)
    {
        _canBus = canBus;
    ...
    public async Task<J1979ResponseFrame> SendAndWaitForResponse(SaeStandardQueryFrame queryFrame, TimeSpan timeout)
    {
        var cancellationTokenSource = new CancellationTokenSource(timeout);
        var tcs = new TaskCompletionSource<J1979ResponseFrame>();
    ...
    private Dictionary<int, J1979ResponseFrame> _received = new();
    ...
    private void OnFrameReceived(object sender, ICanFrame e)
    {
        if (e is StandardDataFrame sdf)
        {
            ...
            var f = new J1979ResponseFrame(sdf);
