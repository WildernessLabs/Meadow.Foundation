...
public partial class MicroGraphics
{
    ...
    /// <summary>
    /// Creates a pixel buffer for the specified color mode and dimensions.
    /// </summary>
    /// <remarks>The returned pixel buffer is optimized for the specified <paramref name="colorMode"/>. 
    /// Ensure that the <paramref name="width"/> and <paramref name="height"/> are appropriate for the intended use
    /// case.</remarks>
    /// <param name="colorMode">The color mode that determines the format of the pixel buffer. Must be a valid <see cref="ColorMode"/> value.</param>
    /// <param name="width">The width of the pixel buffer, in pixels. Must be a positive integer.</param>
    /// <param name="height">The height of the pixel buffer, in pixels. Must be a positive integer.</param>
    /// <returns>An implementation of <see cref="IPixelBuffer"/> that corresponds to the specified <paramref name="colorMode"/> 
    /// and dimensions.</returns>
    /// <exception cref="ArgumentException">Thrown if <paramref name="colorMode"/> is not supported or is set to an invalid value.</exception>
    public static IPixelBuffer CreatePixelBuffer(ColorMode colorMode, int width, int height)
    {
        return colorMode switch
        {
            ...
            ColorMode.FormatUnknown => new BufferRgb332(width, height),
            ...
            _ => throw new ArgumentException($"Unsupported color mode: {colorMode}")
            // TODO: not sure what to do for these
            //ColorMode.Format2bpp = 1 << 1,
            //ColorMode.Format2bppIndexed = 1 << 2,
            //ColorMode.Format2bppGray = 1 << 3,
            //ColorMode.Format4bppIndexed = 1 << 5,
            //ColorMode.Format16bppRgb555 = 1 << 9,
            //ColorMode.Format24bppGrb888 = 1 << 13,
        };
    }
}