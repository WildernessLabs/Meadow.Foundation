...
public abstract class ContinuousRotationServoBase : ServoBase, IContinuousRotationServo
{
    ...
    /// <returns></returns>
    protected TimeSpan CalculatePulseDuration(RotationDirection direction, float speed)
    {
        ...
        if (direction == RotationDirection.None || speed == 0)
        {
            return TimeSpan.FromMilliseconds(midpointPulseDuration / 1000d); ;
        }

        // the delta is how fast; speed * (max - midpoint)
        int midPointPulseDelta = (int)(speed * (Config.MaximumPulseDuration - midpointPulseDuration));
        ...
        Console.WriteLine($"mid: {midpointPulseDuration} delta: {midPointPulseDelta} duration: {pulseDuration}");

        return TimeSpan.FromMilliseconds(pulseDuration / 1000d);
    }
    ...
    protected override void SendCommandPulse(TimeSpan pulseDuration)
    {
