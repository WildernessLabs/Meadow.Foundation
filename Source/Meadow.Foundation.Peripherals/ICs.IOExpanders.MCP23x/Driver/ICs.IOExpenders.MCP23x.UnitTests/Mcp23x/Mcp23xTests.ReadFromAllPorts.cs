...
namespace Meadow.Foundation.ICs.IOExpanders.UnitTests.Mcp23x
{
    public partial class Mcp23xTests
    {
        ...
        private static void MapReadRegistersToSelf(Mock<IMcpDeviceComms> mock)
        {
            mock.Setup(m => m.ReadRegisters(It.IsAny<byte>(), It.IsAny<ushort>()))
                .Returns(
                    (byte address, ushort length) =>
                        Enumerable.Range(0, length).Select(i => (byte) (address + i)).ToArray());
            mock.Setup(m => m.ReadRegister(It.IsAny<byte>())).Returns((byte address) => address);
        }

        [Fact]
        public void ReadFromAllPortsForSingleRegisterSinglePortSucceeds()
        {
            var mcp23x = Mcp23xTestImplementation.Create(
                1,
                1,
                MapReadRegistersToSelf,
                registerMapMockOverride: RegisterMapMockSparseOverride);

            var result = mcp23x.ReadFromAllPorts(Mcp23PortRegister.GPIORegister);
        ...
        [Fact]
        public void ReadFromAllPortsForSingleRegisterSucceeds()
        {
            var mcp23x = Mcp23xTestImplementation.Create(
                4,
                1,
        ...
        public void ReadFromAllPortsInPairedModeSucceeds()
        {
            ...
            var result = mcp23x.ReadFromAllPorts(
                ...
                Mcp23PortRegister.IODirectionRegister,
                Mcp23PortRegister.InterruptControlRegister,
                Mcp23PortRegister.InputPolarityRegister);
